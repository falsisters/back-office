"use client"

import { useState, useRef } from "react"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger, DialogClose } from "@/components/ui/dialog"
import { Alert, AlertDescription } from "@/components/ui/alert"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import type { Price, ProductType } from "../../utils/types/schema.type"

interface AddProductPriceProps {
  productId: string
  onAddPrice: (price: Omit<Price, "id" | "createdAt" | "updatedAt">) => void
}

export function AddProductPrice({ productId, onAddPrice }: AddProductPriceProps) {
  const [newPrice, setNewPrice] = useState<Omit<Price, "id" | "createdAt" | "updatedAt">>({
    price: 0,
    stock: 0,
    type: "FIFTY_KG",
    productId: productId,
  })
  const [error, setError] = useState<string>("")
  const closeRef = useRef<HTMLButtonElement>(null)

  const handleAddPrice = () => {
    setError("")

    if (newPrice.price <= 0) {
      setError("Price must be greater than 0")
      return
    }

    if (newPrice.stock < 0) {
      setError("Stock cannot be negative")
      return
    }

    onAddPrice(newPrice)

    setNewPrice({
      price: 0,
      stock: 0,
      type: "FIFTY_KG",
      productId: productId,
    })
    closeRef.current?.click()
  }

  const handleInputChange = (field: keyof typeof newPrice, value: string | number | ProductType) => {
    setError("")
    setNewPrice((prev) => ({ ...prev, [field]: field === "type" ? value : Number(value) }))
  }

  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button variant="outline">Add Price</Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Add New Price</DialogTitle>
        </DialogHeader>
        <div className="grid gap-4 py-4">
          {error && (
            <Alert variant="destructive">
              <AlertDescription>{error}</AlertDescription>
            </Alert>
          )}
          <div>
            <p className="mb-2 text-sm font-medium">Type:</p>
            <Select value={newPrice.type} onValueChange={(value: ProductType) => handleInputChange("type", value)}>
              <SelectTrigger>
                <SelectValue placeholder="Select type" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="FIFTY_KG">50 KG</SelectItem>
                <SelectItem value="TWENTY_FIVE_KG">25 KG</SelectItem>
                <SelectItem value="FIVE_KG">5 KG</SelectItem>
                <SelectItem value="PER_KILO">Per Kilo</SelectItem>
                <SelectItem value="GANTANG">Gantang</SelectItem>
                <SelectItem value="SPECIAL_PRICE">Special Price</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div>
            <p className="mb-2 text-sm font-medium">Price:</p>
            <Input
              type="number"
              value={newPrice.price}
              onChange={(e) => handleInputChange("price", e.target.value)}
              placeholder="Price"
              min="0"
              step="0.01"
            />
          </div>
          <div>
            <p className="mb-2 text-sm font-medium">Stock:</p>
            <Input
              type="number"
              value={newPrice.stock}
              onChange={(e) => handleInputChange("stock", e.target.value)}
              placeholder="Stock"
              min="0"
            />
          </div>
        </div>
        <div className="flex justify-end gap-4">
          <DialogClose ref={closeRef} asChild>
            <Button variant="outline">Cancel</Button>
          </DialogClose>
          <Button onClick={handleAddPrice}>Add Price</Button>
        </div>
      </DialogContent>
    </Dialog>
  )
}

