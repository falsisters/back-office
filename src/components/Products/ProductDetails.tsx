...
export default function ProductDetails({ product, open, onClose }: ProductDetailsProps) {
        ...
        <DialogHeader>
          <div className="flex items-center gap-4">
            <div className="relative w-16 h-16 rounded-md overflow-hidden flex-shrink-0 border">
              <Image src={product.picture || "/placeholder.svg"} alt={product.name} fill className="object-cover" />
            </div>
            <div>
              <DialogTitle className="text-2xl font-bold">{product.name}</DialogTitle>
              <DialogDescription className="mt-1">Product details and pricing information</DialogDescription>
            </div>
          </div>
        </DialogHeader>
            ...
            {product.SackPrice && product.SackPrice.length > 0 ? (
              <div className="space-y-4">
                {[...product.SackPrice]
                  ...
                  .map((sackPrice) => (
                    <div key={sackPrice.id} className="bg-card rounded-lg border p-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="font-bold text-xl text-primary">₱{sackPrice.price.toFixed(2)}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        <div className="flex items-center gap-2 bg-secondary/5 px-4 py-2 rounded-md border border-secondary/10">
                          ...
                          <p className="font-medium">
                            Stock: <span className="text-lg font-bold text-secondary">{sackPrice.stock} Sacks</span>
                          </p>
                        </div>
              ...
              <div className="bg-muted/30 rounded-lg border p-4 text-center">
                <p className="text-muted-foreground">No sack prices available</p>
              </div>
            )}
          </div>
            ...
            {product.perKiloPrice ? (
              <div className="bg-card rounded-lg border p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="font-bold text-xl text-primary">₱{product.perKiloPrice.price.toFixed(2)}</span>
                </div>

                <div className="flex items-center gap-2 bg-secondary/5 px-4 py-2 rounded-md border border-secondary/10">
                  <Tag className="h-4 w-4 text-secondary" />
                  <p className="font-medium">
                    Stock: <span className="text-lg font-bold text-secondary">{product.perKiloPrice.stock} kg</span>
                  </p>
                </div>
              </div>
            ) : (
              <div className="bg-muted/30 rounded-lg border p-4 text-center">
                <p className="text-muted-foreground">No per kilo price available</p>
              </div>
            )}
          </div>
        </div>
