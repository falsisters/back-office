...
import { getAllTransfers } from "@/lib/server/getAllTransfers"
...
import { GetAllTransfersResponse } from "../../../utils/types/getAllTransfers.type"

export default function TransferHistory() {
  const [transfers, setTransfers] = useState<GetAllTransfersResponse>([])
  const [filteredTransfers, setFilteredTransfers] = useState<GetAllTransfersResponse>([])
  ...
  return (
    <Card className="shadow-md border-t-4 border-t-primary overflow-hidden">
      <CardHeader className="flex flex-row items-center justify-between pb-4 bg-gradient-to-r from-primary/5 to-transparent">
        <CardTitle className="text-3xl font-bold text-primary">Transfer Records</CardTitle>
      ...
      <CardContent className="space-y-8 pt-6 px-6">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-lg text-muted-foreground">Loading transfer history...</p>
          </div>
        ) : error ? (
          ...
          <div className="text-center py-12 border rounded-md bg-muted/20">
            ...
            <p className="text-muted-foreground">No transfer history available for selected date</p>
          </div>
        ) : (
                  ...
                  {typeTransfers.map(transfer => (
                    <div key={transfer.id} className="space-y-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {format(parseISO(transfer.createdAt), "MMM dd, yyyy - h:mm a")}
                      </p>
                    </div>
                  ))}
                </div>
