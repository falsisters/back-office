...
export default function SalesList() {
  ...
  const renderSalesItems = (sales: GetAllSalesByUserIdPayload) => {
    if (viewMode === "perProduct") {
      ...
      return Array.from(productMap.entries()).map(([key, productData]) => {
        ...
        return (
          ...
          >
            <div className="flex justify-between items-center">
              <div className="flex-1">
                <span className="font-medium">
                  {item.product.name} {sackTypeLabel} - {item.quantity}
                  {specialPriceInfo}
                  {gantangInfo}
                </span>
              </div>
  ...
  const mappedSalesData = Object.values(groupedSales).flatMap((dateSales) =>
    dateSales.flatMap((sale) =>
      sale.SaleItem.map((item) => {
        const sackType = item.product.SackPrice[0]?.type as
          | SackType
          | undefined;
