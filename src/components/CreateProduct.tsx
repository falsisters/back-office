...
export function CreateProduct({ onProductCreated }: CreateProductProps) {
  const [newProduct, setNewProduct] = useState<Omit<Product, "id" | "createdAt" | "updatedAt" | "userId">>({
    name: "",
    minimumQty: 1,
  })
  const [prices, setPrices] = useState<Array<Omit<Price, "id" | "createdAt" | "updatedAt" | "productId"> & { profit: Omit<Profit, "id" | "createdAt" | "updatedAt" | "priceId">[] }>>([
    { price: 0, stock: 0, type: "FIFTY_KG", profit: [{ profit: 0 }] }
  ])
  ...
  const handleInputChange = (field: keyof typeof newProduct, value: string | number) => {
    setNewProduct(prev => ({ ...prev, [field]: field === "minimumQty" ? Number(value) : value }))
  }

  const handlePriceChange = (index: number, field: keyof Omit<Price, "id" | "createdAt" | "updatedAt" | "productId">, value: string | number | ProductType) => {
    const updatedPrices = [...prices]
    updatedPrices[index] = {
      ...updatedPrices[index],
  ...
  const handleSubmit = async () => {
    try {
      ...
      const productData = {
        product: {
          name: trimmedName,
          minimumQty: newProduct.minimumQty,
          price: prices
        }
      }
      ...
      // Reset form
      setNewProduct({ name: "", minimumQty: 1 })
      setPrices([{ price: 0, stock: 0, type: "FIFTY_KG", profit: [{ profit: 0 }] }])
