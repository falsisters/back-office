...
export const getProfitsByCashier = async (cashierId: string, date?: string) => {
  ...
  console.log("🔗 API URL:", `${process.env.API_URL}/profit/cashier/${cashierId}?${queryParams.toString()}`);
  console.log("🔗 API Params:", { cashierId, date, queryString: queryParams.toString() });
...
export const getAllProfitsByCashier = async (cashierId: string) => {
  ...
  console.log(`🔄 Fetching profits from ${thirtyDaysAgo.toISOString().split('T')[0]} to ${today.toISOString().split('T')[0]}`);
  ...
  for (let d = new Date(thirtyDaysAgo); d <= today; d.setDate(d.getDate() + 1)) {
    ...
    promises.push(
      getProfitsByCashier(cashierId, dateStr).catch(error => {
        console.log(`⚠️ Failed to fetch profits for ${dateStr}:`, error.message);
  ...
  try {
    ...
    for (const result of results) {
      if (result && result.rawItems) {
        ...
        if (result.sacks?.totalProfit) sackTotal += result.sacks.totalProfit;
        if (result.asin?.totalProfit) asinTotal += result.asin.totalProfit;
      }
    }
    ...
    console.log("✅ Original profit items by date:", 
      allProfitItems.reduce((acc: any, item: any) => {
        const date = item.saleDate ? new Date(item.saleDate).toDateString() : 'Unknown';
        acc[date] = (acc[date] || 0) + 1;
        return acc;
      }, {})
    );
    console.log("✅ Corrected profit items by date:", 
